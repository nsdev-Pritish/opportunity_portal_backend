/**
 * CREDIT MEMO SEARCH — Create & Update service (single file)
 *
 * Writes to credit_memo_search. Both endpoints accept EITHER a single object OR
 * an array (bulk) on the SAME route; every request is normalized to an array,
 ...
 * is stored as-is — same as Invoice / Sales Order / Pipeline. `createdFrom` (the
 * source invoice) is also stored as text + a raw, unresolved internal id rather
 * than a FK, so a credit memo keeps its reference even if the invoice hasn't
 * synced yet — same rationale as invoice_search.so_document_number.
 *
 ...
 * to null (warning).
 *
 * Mirrors src/services/invoiceSearch/invoice.service.ts (same shared batch machinery).
 */
...
/**
 * Fields accepted when creating a credit_memo_search row.
 ...
 * id before storing. documentNumber / dates / amount are stored as-is.
 */
export const CreditMemoCreateSchema = z.object({
  ...
  createdFromInternalId    : z.string().max(50).optional(),
  invoiceDocumentNumber    : z.string().max(100).optional(),
  invoiceDate              : z.string().optional(),
  creditMemoDate           : z.string().optional(),
  // Signed — credit memo amounts can be positive or negative.
...
/**
 * Fields accepted when updating a credit_memo_search row.
 ...
 */
export const CreditMemoUpdateSchema = CreditMemoCreateSchema.partial().extend({
  netsuiteInternalId: z.string({ required_error: 'netsuiteInternalId is required' }).min(1).max(50),
});
...
interface FkResolver {
  ...
  table : any;
  /** credit_memo_search column that receives the resolved DB id */
  column: string;
  /** stable per-table cache key (customers is referenced by 2 fields) */
  tkey  : string;
}
 ...
 * credit_memo_search columns to write, plus warnings for ids that matched no
...
export async function createCreditMemos(records: CreditMemoCreateInput[]) {
  const db = getDb();
  ...
  return processBatch(records, async (rec) => {
    ...
    const [dup] = await db
      .select({ id: creditMemoSearch.id })
      .from(creditMemoSearch)
      .where(eq(creditMemoSearch.netsuiteInternalId, rec.netsuiteInternalId))
      .limit(1);

    if (dup) throw new ConflictError(`Credit memo '${rec.netsuiteInternalId}' already exists`);
    ...
    if (warnings.length) {
      logger.warn({ netsuiteInternalId: rec.netsuiteInternalId, warnings }, 'Unresolved references on credit memo create');
    }
    ...
    const [created] = await db
      .insert(creditMemoSearch)
      .values(row as typeof creditMemoSearch.$inferInsert)
      .returning();
...
export async function updateCreditMemos(records: CreditMemoUpdateInput[]) {
  const db = getDb();
  const cache: FkCache = new Map();
  ...
  return processBatch(records, async (rec) => {
    ...
    const [existing] = await db
      .select({ id: creditMemoSearch.id })
      .from(creditMemoSearch)
      .where(eq(creditMemoSearch.netsuiteInternalId, netsuiteInternalId))
      .limit(1);

    if (!existing) throw new NotFoundError('Credit memo', netsuiteInternalId);

    // Resolve any provided reference fields (NetSuite internal id -> portal DB id).
    const { columns, warnings } = await resolveReferences(db, rec, cache);
    if (warnings.length) {
      logger.warn({ netsuiteInternalId, warnings }, 'Unresolved references on credit memo update');
    }
    ...
    const [updated] = await db
      .update(creditMemoSearch)
      .set(row)
      .where(eq(creditMemoSearch.id, existing.id))
      .returning();
