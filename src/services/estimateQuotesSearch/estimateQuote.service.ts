/**
 * ESTIMATE QUOTE SEARCH — Create & Update service (single file)
 *
 * Writes to estimate_quote_search. Both endpoints accept EITHER a single object
 * OR an array (bulk) on the SAME route; every request is normalized to an array,
...
/**
 * Fields accepted when creating an estimate_quote_search row.
 ...
 */
export const EstimateQuoteCreateSchema = z.object({
...
/**
 * Fields accepted when updating an estimate_quote_search row.
 ...
 */
export const EstimateQuoteUpdateSchema = EstimateQuoteCreateSchema.partial().extend({
  netsuiteInternalId: z.string({ required_error: 'netsuiteInternalId is required' }).min(1).max(50),
});

export type EstimateQuoteCreateInput = z.infer<typeof EstimateQuoteCreateSchema>;
export type EstimateQuoteUpdateInput = z.infer<typeof EstimateQuoteUpdateSchema>;
...
interface FkResolver {
  ...
  table : any;
  /** estimate_quote_search column that receives the resolved DB id */
  column: string;
 ...
 * estimate_quote_search columns to write, plus warnings for ids that matched no
...
export async function createEstimateQuotes(records: EstimateQuoteCreateInput[]) {
  const db = getDb();
  ...
  return processBatch(records, async (rec) => {
    ...
    const [dup] = await db
      .select({ id: estimateQuoteSearch.id })
      .from(estimateQuoteSearch)
      .where(eq(estimateQuoteSearch.netsuiteInternalId, rec.netsuiteInternalId))
      .limit(1);

    if (dup) throw new ConflictError(`Estimate quote '${rec.netsuiteInternalId}' already exists`);
    ...
    if (warnings.length) {
      logger.warn({ netsuiteInternalId: rec.netsuiteInternalId, warnings }, 'Unresolved references on create');
    }
    ...
    const [created] = await db
      .insert(estimateQuoteSearch)
      .values(row as typeof estimateQuoteSearch.$inferInsert)
      .returning();
...
export async function updateEstimateQuotes(records: EstimateQuoteUpdateInput[]) {
  const db = getDb();
  const cache: FkCache = new Map();
  ...
  return processBatch(records, async (rec) => {
    ...
    const [existing] = await db
      .select({ id: estimateQuoteSearch.id })
      .from(estimateQuoteSearch)
      .where(eq(estimateQuoteSearch.netsuiteInternalId, netsuiteInternalId))
      .limit(1);

    if (!existing) throw new NotFoundError('Estimate Quote', netsuiteInternalId);

    // Resolve any provided reference fields (NetSuite internal id -> portal DB id).
    const { columns, warnings } = await resolveReferences(db, rec, cache);
    if (warnings.length) {
      logger.warn({ netsuiteInternalId, warnings }, 'Unresolved references on update');
    }
    ...
    const [updated] = await db
      .update(estimateQuoteSearch)
      .set(row)
      .where(eq(estimateQuoteSearch.id, existing.id))
      .returning();
