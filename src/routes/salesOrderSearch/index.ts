/**
 * SALES ORDER SEARCH module — NetSuite → Portal sync routes
 * Base prefix: /api/v1/sales-orders
 *
 * Two endpoints, both accepting a single OBJECT or an ARRAY (bulk):
 *   POST  /   create record(s)  -> sales_order_search
...
import {
  ...
  createSalesOrders,
  updateSalesOrders,
} from '../../services/salesOrderSearch/salesOrder.service.js';
...
export default async function salesOrderRoutes(app: FastifyInstance) {
  ...
  // POST /api/v1/sales-orders — create (single object or array)
  app.post<{ Body: unknown }>('/', async (req, reply) => {
    const { records, wasSingle } = normalizeToArray(req.body, SalesOrderCreateSchema);
    const result = await createSalesOrders(records);
  ...
  // PATCH /api/v1/sales-orders — update (single object or array)
  app.patch<{ Body: unknown }>('/', async (req, reply) => {
    const { records, wasSingle } = normalizeToArray(req.body, SalesOrderUpdateSchema);
    const result = await updateSalesOrders(records);
