/**
 * CLIENT SHIPPING ADDRESS APIs
 * Shared table: addresses (type = 'shipping')
 *
 *  GET   /api/v1/netsuite/client-shipping-addresses
 *  GET   /api/v1/netsuite/client-shipping-addresses/:nsId
 *  POST  /api/v1/netsuite/client-shipping-addresses
 *  PUT   /api/v1/netsuite/client-shipping-addresses/:nsId
 *  PUT   /api/v1/netsuite/client-shipping-addresses/:nsId/status
 *
 * POST/PUT body:
 * {
...
export default async function shippingAddressRoutes(app: FastifyInstance) {

  // GET /  — list all active shipping addresses with customer NS ID
  app.get('/', async () => {
    const db = getDb();
    const rows = await db
      .select({
  ...
  // GET /:nsId  — single shipping address
  app.get<{ Params: { nsId: string } }>('/:nsId', async (req) => {
    ...
    const [row] = await db
      ...
      .limit(1);
    if (!row) throw new NotFoundError('ShippingAddress', req.params.nsId);
    return row;
  });

  // POST /  — create or upsert shipping address
  ...
  // PUT /:nsId  — update shipping address
  app.put<{ Params: { nsId: string }; Body: unknown }>('/:nsId', async (req) => {
    ...
    const [existing] = await db
      ...
      .limit(1);
    if (!existing) throw new NotFoundError('ShippingAddress', req.params.nsId);
