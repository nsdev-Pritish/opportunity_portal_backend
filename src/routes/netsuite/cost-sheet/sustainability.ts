/**
 * COST SHEET — SUSTAINABILITY APIs
 *
 *  GET   /api/v1/netsuite/cost-sheet/sustainability
 *  GET   /api/v1/netsuite/cost-sheet/sustainability/:nsId
 *  POST  /api/v1/netsuite/cost-sheet/sustainability
 *  PUT   /api/v1/netsuite/cost-sheet/sustainability/:nsId
 *  PATCH /api/v1/netsuite/cost-sheet/sustainability/:nsId/status
 *
 * Body for POST/PUT:
 *  { "netsuiteInternalId": "1", "name": "Organic Cotton" }
 */
...
export default async function sustainabilityCostSheetRoutes(app: FastifyInstance) {
  ...
  app.get<{ Params: { nsId: string } }>('/:nsId', async (req) => {
    ...
    if (!row) throw new NotFoundError('Sustainability', req.params.nsId);
    return row;
  });
  ...
  app.post<{ Body: unknown }>('/', async (req, reply) => {
    ...
    const [created] = await db.insert(sustainabilityOptions).values({
  ...
  app.put<{ Params: { nsId: string }; Body: unknown }>('/:nsId', async (req) => {
    ...
    const [existing] = await db.select({ id: sustainabilityOptions.id }).from(sustainabilityOptions)
      .where(eq(sustainabilityOptions.netsuiteInternalId, req.params.nsId)).limit(1);
    if (!existing) throw new NotFoundError('Sustainability', req.params.nsId);
