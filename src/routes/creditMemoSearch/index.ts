/**
 * CREDIT MEMO SEARCH module — NetSuite → Portal sync routes
 * Base prefix: /api/v1/credit-memos
 *
 * Two endpoints, both accepting a single OBJECT or an ARRAY (bulk):
 *   POST  /   create record(s)  -> credit_memo_search
 ...
 *
 * Mirrors src/routes/invoiceSearch/index.ts.
 */
...
import {
  ...
  createCreditMemos,
  updateCreditMemos,
} from '../../services/creditMemoSearch/creditMemo.service.js';

export default async function creditMemoRoutes(app: FastifyInstance) {
  ...
  // POST /api/v1/credit-memos — create (single object or array)
  app.post<{ Body: unknown }>('/', async (req, reply) => {
    const { records, wasSingle } = normalizeToArray(req.body, CreditMemoCreateSchema);
    const result = await createCreditMemos(records);
  ...
  // PATCH /api/v1/credit-memos — update (single object or array)
  app.patch<{ Body: unknown }>('/', async (req, reply) => {
    const { records, wasSingle } = normalizeToArray(req.body, CreditMemoUpdateSchema);
    const result = await updateCreditMemos(records);
